...
def filter_prediction(disable_valid_filter, disable_extra_one_word_filter, pred_token_2dlist_stemmed, invalidate_unk=True):
    """
    ...
    if not disable_valid_filter:
        is_valid_mask = check_valid_keyphrases(pred_token_2dlist_stemmed, invalidate_unk=invalidate_unk)
...
def main(opt):
    ...
    for data_idx, (src_l, trg_l, pred_l) in enumerate(zip(open(src_file_path, "r", encoding="utf-8"), open(trg_file_path, "r", encoding="utf-8"), open(pred_file_path, "r", encoding="utf-8"))):
        total_num_src += 1
        ...
        filtered_stemmed_pred_token_2dlist, num_duplicated_predictions = filter_prediction(opt.disable_valid_filter, opt.disable_extra_one_word_filter, stemmed_pred_token_2dlist, invalidate_unk=opt.invalidate_unk)
...
def report_classification_scores(score_dict, topk_list, present_tag):
    ...
    for topk in topk_list:
        ...
        output_str += ("#target: {}, #predictions: {}, #corrects: {}\n".format(total_targets_k, total_predictions_k, total_num_matches_k))
